...
VOID Game_Start_Paint(HWND hWnd)
{
	...
	{/*wchar_t str[20] = {};
	HFONT hFont;
	hFont = CreateFont(20, 0, 0, 0, 0, 0, 0, 0, GB2312_CHARSET, 0, 0, 0, 0, TEXT("Î¢ÈíÑÅºÚ"));
	SelectObject(g_mdc, hFont);
	SetBkMode(g_mdc, TRANSPARENT);
	SetTextColor(g_mdc, RGB(50, 255, 50));
	swprintf_s(str, L"sbX%d", g_iX);
	TextOut(g_mdc, 0, 0, str, wcslen(str));
	swprintf_s(str, L"sbY%d", g_iY);
	TextOut(g_mdc, 0, 20, str, wcslen(str));*/

	//BitBlt(g_hdc, 0, 0, WINDOW_WIDTH, WINDOW_HIGHT, g_mdc, 0, 0, SRCCOPY);
		start.DrawMap(hWnd);
		g_tPre = GetTickCount();
